...
internal abstract class AbstractKotlinPlugin(
    ...
    companion object {
        ...
        private fun configureClassInspectionForIC(project: Project) {
            ...
            val inspectTask =
                project.registerTask<InspectClassesForMultiModuleIC>("inspectClassesForKotlinIC") { inspectTask ->
                    inspectTask.archivePath.set(jarTask.map { it.archivePathCompatible.canonicalPath })
                    inspectTask.archivePath.disallowChanges()

                    inspectTask.archiveName.set(jarTask.flatMap { it.archiveFileName })
                    inspectTask.archiveName.disallowChanges()
            ...
            // mustRunAfter breaks circular dependency between jar and inspect tasks
            jarTask.configure { it.mustRunAfter(inspectTask) }
        }
